...
<?php $this->load->view('links/links'); ?>
  ...
  <!-- InstanceBeginEditable name="EditingPane" -->
    <div class="editingwindow">
      <p align="center" class="price">Kodi Builds</p>
      <p align="left"><u>Please upload a banner image for the Kodi builds screen</u></p>
      <div class="trueHalf">
        <p>
      ...
      <?php
      if (file_exists('/var/www/appy.zone/public_html/' . $_SESSION['username'] . '/V5/kodi/banner/image.png')) {
        echo "<img class='img1' style='width:300px;height:55px;border:1px solid red;' src='http://appy.zone/" . $_SESSION['username'] . "/V5/kodi/banner/image.png'/>";
      } 
      else {
      ...
      if(file_exists('/var/www/appy.zone/public_html/' . $username . '/V5/kodi/row1.txt'))
      {
        //print_r($builds);
        foreach(file('/var/www/appy.zone/public_html/' . $username . '/V5/kodi/row1.txt') as $l) 
        {
          $row1[]=$l;
        }
        //print_r($row1);
      ...
      if(file_exists('/var/www/appy.zone/public_html/' . $username . '/V5/kodi/row2.txt'))
      {
        //print_r($builds);
        foreach(file('/var/www/appy.zone/public_html/' . $username . '/V5/kodi/row2.txt') as $l) 
        {
          $row2[]=$l;
        }
        //print_r($row1);
      ...
      if(file_exists('/var/www/appy.zone/public_html/' . $username . '/V5/kodi/row3.txt'))
      {
        //print_r($builds);
        foreach(file('/var/www/appy.zone/public_html/' . $username . '/V5/kodi/row3.txt') as $l) 
        {
          $row3[]=$l;
        }
        //print_r($row1);
      ...
      if(file_exists('/var/www/appy.zone/public_html/' . $username . '/V5/kodi/row4.txt'))
      {
        //print_r($builds);
        foreach(file('/var/www/appy.zone/public_html/' . $username . '/V5/kodi/row4.txt') as $l) 
        {
          $row4[]=$l;
        }
      }
      ...
      if(file_exists('/var/www/appy.zone/public_html/' . $username . '/V5/kodi/row5.txt'))
      {
        //print_r($builds);
        foreach(file('/var/www/appy.zone/public_html/' . $username . '/V5/kodi/row5.txt') as $l) 
        {
          $row5[]=$l;
        }
      }
...
<p>&nbsp;</p>
    ...
    <div class="editingwindow">
      <hr />
      <p><u>Create the Kodi Build Rows</u></p>
      <p>
        <label>
          <input type="checkbox" name="autocomplete" id="autocomplete" <?php 
              if ($autocomplete==1) {
                  echo " checked";
              }
           ?>/>
        </label> 
        Auto generate content from KodiHub
      </p>
    <div id='content'>
      <div class="trueHalf" id='build1'>
        <div align="center">
          ...
          <p>
            <input type="button" name="row1builds" id="row1builds" value="Add Builds" />
          </p>
          <p>
      ...
      ?>'>
        <div align="center">
          ...
          <p>
            <input type="button" name="row2builds" id="row2builds" value="Add Builds" />
          </p>
          <p>
      ...
      ?>'>
        <div align="center">
          ...
          <p>
            <input type="button" name="row3builds" id="row3builds" value="Add Builds" />
          </p>
          <p>
      ...
      ?>'>
        <div align="center">
          ...
          <p>
            <input type="button" name="row4builds" id="row4builds" value="Add Builds" />
          </p>
          <p>
      ...
      ?>'>
        <div align="center">
          ...
          <p>
            <input type="button" name="row5builds" id="row5builds" value="Add Builds" />
          </p>
          <p>
      ...
      <fieldset>
        <div class='overflow-content' style='width:50%;position:relative;float:left;display:block'>
          ...
          <legend>Please select Apps</legend>
            ...
            if(isset($row1))
            {
              foreach($row1 as $broj=>$vr)
              {
                  if($vr==$builds[$i])
                  {
                    $rownew1[$brvr]['title']=$builds[$i];
                    $rownew1[$brvr]['ime']=$buildtitles[$i];
                    $brvr+=1;
                    $pok=1;
                  }
              }
          ...
          ?>
            ...
            <?php endfor; ?>
          <?php else :?>
           <legend>No Kodi builds available</legend>  
          <?php endif;?>        
        </div>
            ...
            foreach($row2 as $broj=>$vr)
              {
                  if($vr==$builds[$i])
                  {
                    $rownew2[$brvr]['title']=$builds[$i];
                    $rownew2[$brvr]['ime']=$buildtitles[$i];
                    $brvr+=1;
                    $pok=1;
                  }
              }
            ...
            foreach($row3 as $broj=>$vr)
              {
                  if($vr==$builds[$i])
                  {
                    $rownew3[$brvr]['title']=$builds[$i];
                    $rownew3[$brvr]['ime']=$buildtitles[$i];
                    $pok=1;
                    $brvr+=1;
                  }
              }
          ...
          <legend>Please select Apps</legend>
            ...
            if(isset($row4))
                ...
                foreach($row4 as $broj=>$vr)
              {
                  if($vr==$builds[$i])
                  {
                    $rownew4[$brvr]['title']=$builds[$i];
                    $rownew4[$brvr]['ime']=$buildtitles[$i];
                    $pok=1;
                    $brvr+=1;
                  }
              }
            ...
            foreach($row5 as $broj=>$vr)
              {
                  if($vr==$builds[$i])
                  {
                    $rownew5[$brvr]['title']=$builds[$i];
                    $rownew5[$brvr]['ime']=$buildtitles[$i];
                    $pok=1;
                    $brvr+=1;
                  }
              }
...
$(document).ready(function(){
  ...
  {     
    ...
    formData.append('autocomplete',$("#autocomplete").is(':checked'));
    ...
    formData.append('buildcount','<?php if(isset($builds)) echo count($builds); else echo 0;?>');    
      //if (filesArray.length == 1) {
          $.ajax({
              url: '<?php echo base_url(); ?>appy/kodibuildsupload', 
              type: "POST",            
              data: formData,
              contentType: false,       
              cache: false,             
