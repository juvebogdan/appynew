  ...
  $('#manualtitle').on('change', function() {
      ...
      $.ajax({         
        type: 'POST',
        url: "http://appy.zone/appynew/playlist/populate/series",
        data: formData,
        contentType: false,       
        cache: false, 
        dataType: 'json',            
  ...
  $("#submit").click(function(e) {
    ...
    if (!$("#load2").hasClass("fa fa-circle-o-notch fa-spin")) {
      $.ajax({         
        type: 'POST',
        url: "http://appy.zone/appynew/playlist/imdb/submitseries",
        data: formData,
        contentType: false,       
        cache: false, 
        dataType: 'json',            
  ...
  $("#save3").click(function(e) {
    ...
    if (filesArray3.length == 1 && $('#movies').val()!='') {
        if (!$("#load3").hasClass("fa fa-circle-o-notch fa-spin")) {
          $.ajax({         
            type: 'POST',
            url: "http://appy.zone/appynew/playlist/uploadimage/series",
            data: formData,
            contentType: false,       
            cache: false,             
            processData:false,
  ...
  $("#manualsubmit").click(function(e) {
    ...
    if (!$("#load4").hasClass("fa fa-circle-o-notch fa-spin")) {
      $.ajax({         
        type: 'POST',
        url: "http://appy.zone/appynew/playlist/manualedit/series",
        data: formData,
        contentType: false,       
        cache: false, 
        dataType: 'json',            
